...
class Services_Akismet_TinyHttpException extends ErrorException {}
 ...
 *
 *      - **debug** - Print the HTTP request before making it to Akismet
 ...
 *     );
 */
class Services_Akismet_TinyHttp {
		...
		switch ($name) {
			...
			case 'put':
				...
				if (strlen($req_body)) {
					...
					} else throw new Services_Akismet_TinyHttpException('unable to open temporary file');
				}
				break;
			case 'head':
			...
			if ($curl = curl_init()) {
				if (curl_setopt_array($curl, $opts)) {
					if ($response = curl_exec($curl)) {
						...
						throw new Services_Akismet_TinyHttpException(curl_error($curl));
					}
				} else throw new Services_Akismet_TinyHttpException(curl_error($curl));
			} else throw new Services_Akismet_TinyHttpException('unable to initialize cURL');
